...
class ProductBatchInline(admin.TabularInline):
    model = ProductBatch
    extra = 0
    fields = ['batch_number', 'branch', 'purchase_date', 'cost_price', 'selling_price', 'quantity_received', 'quantity_remaining', 'is_depleted']
    readonly_fields = ['batch_number', 'branch', 'purchase_date', 'cost_price', 'selling_price', 'quantity_received', 'quantity_remaining', 'is_depleted']
    can_delete = False
...
class DeviceComponentWarrantyInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['component_type', 'component_name', 'warranty_months', 'warranty_start_date', 'warranty_expiry_date', 'status', 'claim_count']
    readonly_fields = ['component_type', 'component_name', 'warranty_months', 'warranty_start_date', 'warranty_expiry_date', 'claim_count']
    can_delete = False
...
class VendorRMAClaimItemInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['product', 'defective_serial_or_imei', 'defect_description', 'resolution', 'replacement_batch_or_serial', 'credit_amount']
    show_change_link = True
...
class ProductAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'tax_pricing_type_badge', 'mdms_default_badge', 'is_spare_part_badge', 'requires_imei_tracking', 'rack_number'
    ]
    list_filter = [
        'default_mdms_status', 'tax_pricing_type', 'is_spare_part', 'category', 'brand', 'network_type',
        'requires_imei_tracking', 'is_vat_applicable', 'created_at'
    ]
    search_fields = ['name', 'sku', 'barcode', 'model_name', 'model_number', 'processor_chipset', 'rack_number']
    inlines = [
    ...
    def barcode_display(self, obj):
        if obj.barcode:
            return format_html('<span class="font-monospace text-primary font-bold">{}</span>', obj.barcode)
    ...
    display_variant_info.short_description = "Variant (RAM/ROM)"
    ...
    tax_pricing_type_badge.short_description = "Tax Mode"
    ...
    mdms_default_badge.short_description = "Default MDMS"
    ...
    is_spare_part_badge.short_description = "Item Type"
...
class ItemInstanceAdmin(admin.ModelAdmin):
    ...
    def imei_2_status_badge(self, obj):
        ...
        if obj.imei_2 and obj.imei_2.strip() != '':
            return format_html(
                '<span style="color: #065f46; background-color: #ecfdf5; border: 1px solid #a7f3d0; padding: 2px 7px; border-radius: 999px; font-weight: 700; font-size: 10px;"><i class="fas fa-check-circle me-1"></i> CAPTURED</span>'
            )
        if obj.imei_2_pending_scan:
            return format_html(
                '<span style="color: #92400e; background-color: #fef08a; border: 1px solid #eab308; padding: 2px 7px; border-radius: 999px; font-weight: 700; font-size: 10px;"><i class="fas fa-triangle-exclamation me-1"></i> PENDING SCAN</span>'
            )
        return format_html('<span style="color: #64748b; font-size: 10px;">SINGLE SIM</span>')
    imei_2_status_badge.short_description = "IMEI 2 Status"
    ...
    def condition_badge(self, obj):
        ...
        return format_html('<span style="color: white; background-color: {}; padding: 3px 8px; border-radius: 999px; font-weight: 600; font-size: 11px;">{}</span>', color, obj.get_condition_display())
    condition_badge.short_description = "Condition"

    def mdms_status_badge(self, obj):
        if obj.mdms_status == 'REGISTERED_OFFICIAL':
            return format_html('<span style="color: #065f46; background-color: #ecfdf5; border: 1px solid #a7f3d0; padding: 3px 8px; border-radius: 999px; font-weight: 700; font-size: 10.5px;"><i class="fas fa-check-circle me-1"></i> MDMS OK</span>')
        elif obj.mdms_status == 'GRAY_UNREGISTERED':
            return format_html('<span style="color: #991b1b; background-color: #fef2f2; border: 1px solid #fecaca; padding: 3px 8px; border-radius: 999px; font-weight: 700; font-size: 10.5px;"><i class="fas fa-exclamation-triangle me-1"></i> GRAY MARKET</span>')
        elif obj.mdms_status == 'INDIVIDUAL_CUSTOMS_PAID':
            return format_html('<span style="color: #1e40af; background-color: #eff6ff; border: 1px solid #bfdbfe; padding: 3px 8px; border-radius: 999px; font-weight: 700; font-size: 10.5px;">CUSTOMS PAID</span>')
    ...
    mdms_status_badge.short_description = "NTA MDMS"

    def source_type_badge(self, obj):
        if obj.source_type == 'CUSTOMER_EXCHANGE_TRADE_IN':
            return format_html('<span style="color: #92400e; background-color: #fffbeb; border: 1px solid #fde68a; padding: 2px 7px; border-radius: 999px; font-weight: 700; font-size: 10px;">TRADE-IN</span>')
        elif obj.source_type == 'NEW_PURCHASE_GRN':
            return format_html('<span style="color: #065f46; background-color: #ecfdf5; padding: 2px 7px; border-radius: 999px; font-weight: 700; font-size: 10px;">BRAND NEW</span>')
        return format_html('<span style="color: #475569; font-size: 10px;">{}</span>', obj.get_source_type_display())
    source_type_badge.short_description = "Source Origin"
...
class DeviceComponentWarrantyAdmin(admin.ModelAdmin):
    ...
    search_fields = ['item_instance__imei_1', 'item_instance__imei_2', 'item_instance__serial_number', 'item_instance__customer_name', 'component_name']
    ...
    def status_badge(self, obj):
        ...
        return format_html('<span style="color: white; background-color: {}; padding: 3px 8px; border-radius: 999px; font-weight: 600; font-size: 11px;">{}</span>', color, obj.get_status_display())
    status_badge.short_description = "Warranty Status"
...
class VendorRMAClaimAdmin(admin.ModelAdmin):
    ...
    def status_badge(self, obj):
        ...
        color = colors.get(obj.status, '#64748b')
        return format_html('<span style="color: white; background-color: {}; padding: 3px 8px; border-radius: 999px; font-weight: 600; font-size: 11px;">{}</span>', color, obj.get_status_display())
    status_badge.short_description = "RMA Status"
...
class BranchStockAdmin(admin.ModelAdmin):
    list_display = ['product', 'branch', 'quantity', 'reserved_quantity', 'quarantined_defective_quantity', 'low_stock_threshold']
...
class StockMovementLogAdmin(admin.ModelAdmin):
    list_display = ['created_at', 'movement_type', 'product', 'branch', 'quantity_delta', 'new_quantity', 'reference_document', 'imei_or_serial_number', 'user']
