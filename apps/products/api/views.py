...
def get_product_avatar_meta(product):
    ...
    # Condition A: Explicit IMEI tracking enabled
    is_phone = bool(product.requires_imei_tracking)

    # Condition B: Category name contains mobile/phone keywords
    if not is_phone and any(k in cat_name or k in cat_name_np for k in ['phone', 'mobile', 'smartphone', 'ह्यान्डसेट', 'स्मार्टफोन']):
        is_phone = True

    # Condition C: Product has explicit RAM or Internal Storage specified (e.g. 128GB, 256GB, 8GB)
    ...
    # Condition D: Pattern matching for phone naming conventions (e.g. '15 128', '16 128', 'iPhone', 'Galaxy', 'Redmi', 'Note', 'Pro Max')
    if not is_phone:
        phone_patterns = [
            ...
            r'\b(1[1-6]|se)\s+(128|256|512|64|1tb)\b',  # e.g., '15 128', '16 256'
...
class ProductSearchAPIView(APIView):
    """
    High-Speed POS Counter, GRN & Service Intake Lookup API.
    Searches across Primary IMEI 1, Secondary IMEI 2, Serial Number, Device UID,
    Product Barcode, Package Barcode, Model Name, Model Number, RAM/Storage variant, and SKU.
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 1. Dual-IMEI / Serial / Device UID Exact Handset Match
        ...
        # 2. Match Packaging Barcode (Bulk Box Conversions)
        pkg_match = UnitConversion.objects.filter(barcode=query).select_related(
        ...
        # 3. Product Catalog General Lookup (Bundles in-stock IMEI pairs for the active branch)
        products = Product.objects.filter(
            ...
            is_active=True
        ).select_related('base_unit', 'category', 'brand').prefetch_related('component_warranty_rules', 'branch_stocks')[:25]
        ...
        for p in products:
            ...
            # Fetch active in-stock IMEI units for this handset at this branch
            in_stock_units = []
